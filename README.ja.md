...
**既存の文書 — 要件定義、API リファレンス、モックアップ、図 — がある場合は、実行前に `specs/1.0.0/` の下にコピーしてください。** フォルダ名は自由で、ステージ0 がそれらを見つけ、どれが仕様そのもので、どれが仕様を説明するだけかを尋ねます。詳細は [`docs/adopting.ja.md`](./docs/adopting.ja.md) の手順2 にあります。
