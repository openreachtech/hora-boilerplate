...
**A listed section names nothing to build, and listing one was not a horizon decision.** It belongs in none of the three lists: not "built this time", because this version builds no part of it and accepts no part of it; not "out of scope for now", because it is already running and there is nothing to unblock; not "permanently out of scope", because nothing was excluded from the design — the code is there and the data model carries its rows. **What this stage does with it is leave it alone.** The version that next changes the feature writes its blocks and decides its horizon then, and until that happens the annotation is the whole record (`../../hora/references/spec-format.md`, "`baseline`"). Pulling one into "for now" would leave a seam kept open for code that already exists.
...
**Unreachable in either direction is a defect, and both directions are checked here.** An operation no screen calls is either a missing screen or a feature nobody wants; a button with no operation behind it is a screen designed against a backend that does not exist. The acceptance review looks for exactly these two, at the far end of eighteen checkpoints. **Both are read over what this version specifies, and a listed section sits outside them** — its operations were recorded by stage 4 and justified by the feature's name, and its screens are inherited lines with no `Calls` table, so read as ordinary rows every one of those operations is reachable from no screen: twelve false defects on a twelve-operation feature, and the run then either sends stage 4 back to design a screen for an operation that has served users for a year, or records this exit condition as met while its own words say it is not. **The running product already connects the two**, and neither side restates the connection here for it to stay true (`../../hora-spec-frontend/SKILL.md`, which states the same reach).
...
**A listed feature's operations get this stage's full work, undiminished.** Listing suspends exactly two checks, and neither is this stage's (`../../hora/references/spec-format.md`, "`baseline`"): every operation the feature exposes still states its kind, its caller and its refusal through the batched per-endpoint check, and **anything reachable without authentication is still asked about one operation at a time**, never batched, whatever the declaration says. The code is running and already reachable — a caller nobody stated is a filter somebody copied off a neighbour, and that is as true of inherited code as of new code.

**What changes is only where the refusal is written: the operation's own row, never an appended criterion.** A listed section has no `<!-- acceptance -->` block to append to, and writing one for it would produce a criterion nothing will ever verify — indistinguishable afterwards from a criterion somebody accepted, which is the failure the whole declaration is built to avoid. The row states the refusal; the version that pays the debt writes the block and this stage's refusal criteria land in it then.
...
**A listed section is mechanical in both directions, and the second direction is the one worth stating.** That it carries neither block raises nothing — that is what the declaration bought. That it carries *anything else* is a stop: a usecases block, an acceptance block, a screen section or a data-model table of its own means somebody specified a feature the document says nobody will verify, and the two readings of that section now disagree (`../../hora/references/spec-format.md`, "`baseline`").

**Counting them is part of the exit condition because a count is the only thing that keeps the reach visible.** A document with three of twenty features listed and no statement of that reads exactly like a document with none — and the count is what `/hora-plan`'s `## Not accepted` entries and every acceptance verdict's `not-accepted:` line are then checked against, so that no verdict downstream reads a bare `passed` (`../../hora-plan/SKILL.md`; `../../hora-accept/SKILL.md`).
