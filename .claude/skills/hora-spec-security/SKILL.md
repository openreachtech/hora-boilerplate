...
description: Stage 6 of /hora-spec. Give every operation and every surface a stated caller and a stated refusal, name every piece of personal or regulated data, and record why the role-or-endpoint split went the way it did. The one gate with no not-applicable case. Invoked by /hora-spec, or directly.
---
...
**This stage is never not applicable.** A release with no authentication still has to say so, and why. An unstated caller is an operation that will be implemented with whatever filter its neighbours had, and nothing in the resulting code says nobody ever decided.
...
Those two are one word apart and are not the same claim. "Anyone with a session token can call `deleteAccount`" is something the code says; the same sentence with *should* is something no code can say — and on a product that already runs it is usually something nobody ever said either.
...
| a new surface | stage 5 |
| how an auth filter is wired, or how a public operation is allowed through | the conventions package |
| whether the built code has a defect | **the audit skills**, at checkpoint 8. They audit code; this stage writes what that audit checks against |
| a token lifetime, a hashing algorithm, a header | the conventions package, at checkpoint 6 |

---

## The pass

**Walk every operation from stage 4 and every surface from stage 5. No exceptions, no sampling.**
...
**Under `Authority: as-built` the walk stays complete but the confirming batches.** For those features, "who may call each of these today" goes up as one check per endpoint — a table of operations and their current callers, corrected rather than composed.

**The exception that never batches: anything reachable without authentication.** An operation callable before signing in, a surface that renders to nobody-in-particular, an allowlisted public operation — **each is put up alone, by name, whatever the declaration says.** "Anyone can call this today" is a fact; "anyone may call this" is a decision, and it is the one decision `as-built` must not make silently.

**`Baseline: inventoried` reaches nothing in this stage.** All it lowers is that a listed feature owes no use cases and no acceptance criteria, and neither block ever answered any of the questions above. **Every operation a listed feature exposes still names its caller and its refusal.** What changes is only where the refusal is written — **the operation's own row**, since a listed section has no acceptance block to append to.
...
The next version's new role is decided against that reason or against nothing. It goes in `.hora/spec/<version>/_stages.md`.

---

## The kinds of defect to ask about

**The audit skills are the authority on what kinds exist** — match them against the equipped descriptions, invoke them, and read the list. They audit code, not documents, so what is borrowed is the list of kinds, never a verdict.
...
| exposure | which servers and data stores are reachable from outside, and which must never be |
| secrets | what secrets does this need, and who issues them |
| injection | which inputs reach a query, a shell, a template, or another system |
...
**The caller goes on the operation, in the section that already holds it** — not in a security appendix. A permission written elsewhere is a permission nobody reads next to the thing it governs.

```markdown
| Operation | Input | Result | Kind | Caller |
|---|---|---|---|---|
| `attendances` | `AttendancesInput(month)` | `AttendancesResult` | query | a member of staff, own records only. A manager, for their own team |
...
| Exposure | only the two servers are reachable. The data store and the queue are not |
```

### The acceptance criteria this stage adds

**A refusal is a behavior, so it is testable, so it belongs in an acceptance block.** Add one per operation whose refusal matters — **except on a listed section, where it stays in the operation's own row.**
...
Every operation naming its caller and its refusal; every surface naming its actor; every personal or regulated field named; the endpoint split's reason written.
...
| an operation needs splitting so permissions can differ | **stage 4** |
| a surface needs splitting for the same reason | **stage 5** |
