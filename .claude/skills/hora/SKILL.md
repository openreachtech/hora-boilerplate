...
| `references/spec-template.md` | **the authority on the format** of `specs/<version>/spec.md`. Explains it; is not the thing filled in |
| `references/spec-skeleton.md` | **the blank spec.** Headings and table headers only. A human copies this to `specs/<version>/spec.md` |
...
When a human asks about the format of `specs/`, point them at `references/spec-template.md` to read and `references/spec-skeleton.md` to copy. **No hora skill copies it there itself** — `specs/1.0.0/spec.md` ships empty, and writing the first spec is a human's job.
