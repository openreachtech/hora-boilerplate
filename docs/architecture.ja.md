...
| **3–7, 10, 12–16** | `hora-implementer` | 通常の実装。1関所ぶんのファイルに範囲を絞る — 関所 3・5・6・12 では1単位ぶんに絞り、テーブル／モジュール／操作／コンポーネントごとに1体 |
...
| `.hora/` | その作業を記録する skill、および自分が導いたダイジェスト1本だけを書く `hora-digester` | 人間は読むだけ |
...
**2つの機能は並行して走らず、2つの関所も並行しません。** 1つの関所の内側では、その単位が並行します — テーブルごと、モジュールごと、操作ごと、コンポーネントごとに1体 — そしてこの2つの主張の距離が、この節の主題です。
